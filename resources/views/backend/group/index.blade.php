...
@section('title','Ecommerce Laravel || Group Page')
          ...
          <tbody>
            @foreach($groups as $brand)
                ...
                <tr>
                    ...
                    <td>
                        <a href="{{route('group.edit',$brand->id)}}" class="btn btn-primary btn-sm float-left mr-1" style="height:30px; width:30px;border-radius:50%" data-toggle="tooltip" title="edit" data-placement="bottom"><i class="fas fa-edit"></i></a>
                        <form method="POST" action="{{route('group.destroy',[$brand->id])}}">
                          @csrf
                          @method('delete')
